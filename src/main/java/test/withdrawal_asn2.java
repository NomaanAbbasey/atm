...
class withdrawal_asn2 {
  ...
  //The below 5 tests are checking the balance boundaries for non-students 
    ...
    @Test
    public void testCalculateWithdrawalFee() {
        int withdrawalAmount = 10000; // nominal value, amount withdrawn has no dependencies
        int nominalBalance = 10000;

        assertEquals(0, calc.calculateWithdrawalFee(withdrawalAmount, nominalBalance, true, 7));
        assertEquals(
            Math.round(0.001 * withdrawalAmount),
            calc.calculateWithdrawalFee(withdrawalAmount, nominalBalance, true, 3)
        );
        assertEquals(
            Math.round(0.002 * withdrawalAmount),
            calc.calculateWithdrawalFee(withdrawalAmount, 50000, false, 3)
        );
        assertEquals(
            Math.round(0.001 * withdrawalAmount),
            calc.calculateWithdrawalFee(withdrawalAmount, 500000, false, 3)
        );
